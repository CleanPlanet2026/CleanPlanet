...
모든 PR에서 변경 내용이 `docs/PROJECT_PROGRESS.md`에 영향을 주는지 확인한다.

다음 변경이 있으면 해당 PR에서 진행 현황 문서를 반드시 함께 갱신한다.

- 기능 또는 시스템의 추가, 변경, 제거
- 게임 흐름, UI, 조작법 또는 사용자 안내 변경
- 개발 환경, 빌드, 배포 방식 변경
- 검증 상태 또는 남은 우선 작업 변경

기준일, 기준 브랜치와 main 커밋, 시스템별 구현 상태, 검증 상태 및 남은 작업이 실제 저장소 상태와 일치하도록 유지한다.
